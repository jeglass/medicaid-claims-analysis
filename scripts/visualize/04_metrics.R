...
make_metrics_fig <- function(monthly_metrics, title_html) {
  fig <- plot_ly() |>
    add_trace(data = monthly_metrics, x = ~year_month, y = ~TOTAL_CLAIMS,
              name = "Total Claims", type = 'scatter', mode = 'lines+markers',
              line = list(width = 2.5, color = '#667eea'),
              marker = list(size = 7, color = '#667eea'),
              text = ~paste0("<b>Total Claims</b><br>",
                             "Claims: ", format(TOTAL_CLAIMS, big.mark = ","), "<br>",
                             "Date: ", format(year_month, "%Y-%m")),
              hoverinfo = 'text') |>
    add_trace(data = monthly_metrics, x = ~year_month, y = ~TOTAL_UNIQUE_BENEFICIARIES,
              name = "Unique Beneficiaries", type = 'scatter', mode = 'lines+markers',
              line = list(width = 2.5, color = '#f093fb'),
              marker = list(size = 7, color = '#f093fb'),
              text = ~paste0("<b>Unique Beneficiaries</b><br>",
                             "Beneficiaries: ", format(TOTAL_UNIQUE_BENEFICIARIES, big.mark = ","), "<br>",
                             "Date: ", format(year_month, "%Y-%m")),
              hoverinfo = 'text') |>
    add_trace(data = monthly_metrics, x = ~year_month, y = ~TOTAL_PAID,
              name = "Total Paid ($)", type = 'scatter', mode = 'lines+markers',
              yaxis = 'y2',
              line = list(width = 2.5, color = '#4facfe'),
              marker = list(size = 7, color = '#4facfe'),
              text = ~paste0("<b>Total Paid</b><br>",
    ...
    layout(
      ...
      yaxis = list(title = "<b>Claims / Beneficiaries</b>", rangemode = "tozero", side = "left"),
      yaxis2 = list(title = "<b>Total Paid ($)</b>", rangemode = "tozero",
                    overlaying = "y", side = "right"),
      hovermode = 'closest',
      legend = list(title = list(text = "<b>Metrics</b>"), orientation = "v",
                    x = 1.15, y = 1, bgcolor = "rgba(255,255,255,0.8)",
...
fig_all <- make_metrics_fig(
  monthly_metrics_all,
  paste0("<b>ALL CHESTNUT PROVIDERS - AGGREGATED</b><br>",
...
metrics_legend_html <- paste0(
  ...
  format(totals$claims, big.mark = ","), '</div>\n',
  ...
  format(totals$beneficiaries, big.mark = ","), '</div>\n',
  ...
  format(round(totals$paid), big.mark = ","), '</div>\n',
...
html <- make_dashboard_html(
  ...
  card_stats_fn = function(info) {
    paste0(
      "Claims: ", format(info$claims, big.mark = ","), " | ",
      "Beneficiaries: ", format(info$beneficiaries, big.mark = ","), " | ",
      "Paid: $", format(round(info$paid), big.mark = ",")
    )
  }
)
...
cat("  Total beneficiaries:", format(totals$beneficiaries, big.mark = ","), "\n")
cat("  Total paid: $", format(round(totals$paid), big.mark = ","), "\n", sep = "")
