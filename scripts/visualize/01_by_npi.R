...
fig_all <- plot_ly(
  ...
  layout(
    ...
    hovermode = 'closest',
    legend = list(
      title = list(text = "<b>HCPCS Code</b>"),
      orientation = "v",
      x = 1.02,
      y = 1,
...
for (i in 1:nrow(billing_npis)) {
  ...
  # Get top 20 HCPCS codes for this NPI (sum actual TOTAL_CLAIMS, not count rows)
  top_codes <- npi_data |>
    group_by(HCPCS_CODE, code_description) |>
    summarize(total = sum(TOTAL_CLAIMS, na.rm = TRUE), .groups = "drop") |>
    arrange(desc(total)) |>
  ...
  cat("  Top 20 codes:", paste(head(top_codes$HCPCS_CODE, 10), collapse = ", "), "...\n")

  # Aggregate by month and HCPCS code (for top 20 codes only)
  # NOTE: Data is already aggregated, each row has a TOTAL_CLAIMS value
...
<html>
<head>
    <title>Chestnut Health Systems - HCPCS Trends by Provider</title>
...
<body>
    <div class="header">
        ...
        <p>Top 20 HCPCS Codes by Provider Over Time</p>
        <p style="font-size:14px; opacity:0.8;">Interactive time series showing claim volumes by month</p>
    </div>
...
html_content <- paste0(html_content, plots_html, '
    ...
    <div class="footer">
        ...
        Data: Chestnut Health Systems Medicaid Claims
    </div>
...
output_file <- "output/graphs/chestnut_all_providers.html"
