...
provider_totals <- chestnut_claims |>
...
provider_code_counts_wide <- chestnut_claims |>
  filter(HCPCS_CODE %in% recent_chestnut_hcpcs) |>
...
provider_summary_wide <- provider_totals |> 
  ...
  {
    n_cols <- paste0("n_", recent_chestnut_hcpcs)
    n_cols <- intersect(n_cols, names(.))
    mutate(
      .,
      across(
        all_of(n_cols),
        ~ if_else(total_claims > 0, .x / total_claims, 0),
        .names = "prop_{.col}"
      )
    )
  } |> 
  rename_with(~ sub("^prop_n_", "prop_", .x), starts_with("prop_n_"))

chestnut_claims <- wa_or_filtered |>
